...
const ModelDetail: React.FC = () => {
  ...
  if (!model) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white font-rethink">
        <SEO 
          title="Model Not Found | AI Inventory"
          description="The requested AI model was not found. Browse our comprehensive database of AI models."
        />
        Model not found
      </div>
    );
  ...
  // Structured data for model page
  ...
  return (
    <div className="min-h-screen w-full relative font-rethink">
      <SEO
        title={`${model.name} - ${model.description} | AI Inventory`}
        description={`Learn about ${model.name}, an AI model by ${model.provider}. ${model.description} Features: ${model.features.join(', ')}.`}
        keywords={`${model.name}, ${model.provider}, AI model, ${model.features.join(', ')}, artificial intelligence`}
        url={window.location.href}
        image={model.imageUrl}
        type="article"
