...
// Icons removed as they are replaced by Header component
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        const response = await modelApi.getAll({ limit: 20, page: 1, sortBy: 'random' });
  ...
  return (
    <div className="min-h-screen w-full bg-black text-gray-100 font-rethink flex flex-col relative">
      ...
      {showScroll && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 z-40 p-4 bg-cyan-600/80 backdrop-blur-md rounded-2xl shadow-2xl hover:bg-cyan-500 transition-all duration-300 group border border-cyan-400/30"
      ...
      <div className="flex-1 flex flex-col justify-center items-center py-20 relative z-10 mt-16">
        <div className="text-center mb-16 w-full relative max-w-5xl mx-auto px-4">
          {/* Hero Badge */}
          <div className="inline-flex items-center px-4 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/20 mb-8 backdrop-blur-md animate-fadeIn">
            <span className="w-2 h-2 bg-cyan-400 rounded-full mr-2 animate-pulse shadow-[0_0_10px_rgba(34,211,238,0.5)]"></span>
            <span className="text-cyan-300 text-sm font-medium tracking-wide">Updated with latest AI models</span>
          </div>
          
          {/* Main Title */}
          {/* Main Title */}
          <h1 className="text-4xl md:text-6xl lg:text-8xl font-bold mb-6 md:mb-8 font-outfit tracking-tight animate-slideInLeft">
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-gray-200 to-gray-400 drop-shadow-xl">
              LLM Models
            </span>
            <br />
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-violet-400 to-fuchsia-400 animate-gradient-x">
              Explorer
            </span>
          </h1>
          
          {/* Subtitle */}
          <p className="text-gray-400 text-base md:text-xl font-rethink max-w-2xl mx-auto leading-relaxed mb-8 md:mb-10 animate-slideInRight px-4">
            Discover and compare the world's most advanced language models. 
            Your gateway to the future of artificial intelligence.
          </p>
          
          {/* CTA Buttons */}
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-5 justify-center animate-fadeIn px-4" style={{ animationDelay: '0.2s' }}>
            <button 
